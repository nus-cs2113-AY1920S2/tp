...
A `ScheduleHandler` object can be created by the `CommandHandler` class with the `ScheduleHandler#ScheduleHandler()` constructor, and is passed an ArrayList of `Contact`s.
