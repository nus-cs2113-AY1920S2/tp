...
/**
 * Sort all the undo tasks in terms of keywords of ModuleCode and category by either deadline or priority and display the sorted tasks to the user.
 * By default, the tasks will be sorted based on deadline by default; unless, users specify the need to sort all undo tasks by priority.
 */
public class ListTaskSortedCommand extends ListCommand {
    public static final String COMMAND_WORD = "lsts";
    public static final String FORMAT = COMMAND_WORD + " [ -m <module keyword> -c <category keyword> ] [ -d -p (choose 1; default -d) ]";
    ...
    public CommandResult execute() {
        ArrayList<Task> filteredTaskList = createFilteredTaskList(moduleCode, category, NO_KEYWORD, false, false).stream()
